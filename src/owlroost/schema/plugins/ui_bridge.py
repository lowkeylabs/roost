...
class OwlUiBridgePlugin:
    """
    Harvest schema-relevant signals from owlplanner.config.ui_bridge.

    We only register *config-facing* fields that OWL accepts via config,
    not UI-only flattened keys.

    Sources:
      - ACA_FIELDS → aca_settings.*
      - bootstrap_sor handling → rates_selection.bootstrap_type, block_size
      - explicit reads/writes in ui_bridge → rates/tax knobs
      - account allocation mode → asset_allocation.{taxable, tax_deferred, tax_free, hsa}
    """
    ...
    def register(self, registry):
        ...
        # -------------------------------------------------
        def register_if_missing(name, path, dtype=object, description=""):
        ...
        # 1) ACA fields
        # -------------------------------------------------
        aca_fields = getattr(ub, "ACA_FIELDS", [])
        for key in aca_fields:
            register_if_missing(
                name=f"aca_settings.{key}",
                path=("aca_settings", key),
