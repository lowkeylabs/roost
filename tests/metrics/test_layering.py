...
def test_aggregate_metrics_use_aggregate_projection():
    ...
    aggregates = [m for m in reg.all() if "__" in m.name]

    assert aggregates

    for metric in aggregates:
        assert metric.projection_kind == "aggregate"


def test_synthetic_metrics_use_synthetic_projection():
    for metric in CANONICAL_METRICS:
        if metric.projection_kind == "synthetic":
            assert metric.projection_kind == "synthetic"
